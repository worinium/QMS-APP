    ...
    public partial class CurrentTicketSection : UserControl
    {
        ...
        public void AnnounceTicket()
        {
            try
            {
                ...
                player.Open(new System.Uri(AppDomain.CurrentDomain.BaseDirectory + @"\Content\Sounds\announcer.wav"));
                player.Play();
            ...
            Application.Current.Dispatcher.Invoke(DispatcherPriority.Background, new ThreadStart(delegate
            {
                ...
                foreach (var wn in Application.Current.Windows)
                {
                    if (wn.GetType() == typeof(GUI.WaitingRoomScreen))
                    {
                        ...
                        {
                            foreach (var view in region.Views)
                                if (view.GetType() == typeof(GUI.Views.StreamSection))
                                {
                                    streamUC = ((GUI.Views.StreamSection)view);
                                }
                        }
                    }
                }
